...
public class Wall extends Map2DElement {
    ...
    private void setBeg(int len) {
        int x = this.getLocation().getX(), y = this.getLocation().getY();
        if (this.dir == Direction.W) x -= len - 1;
        else if (dir == Direction.N) y -= len - 1;
        this.beg = new Int2DPoint(x, y);
    }

    private static Int2DSpan getSpan(int len, Direction dir) {
        if (dir == Direction.W || dir == Direction.E) return new Int2DSpan(len, 1);
        else return new Int2DSpan(1, len);
    }
    ...
    public Wall(int x, int y, int len, Direction dir) {
        super(new Int2DPoint(x, y), getSpan(len, dir));
        this.dir = dir;
        this.setBeg(len);
    }
